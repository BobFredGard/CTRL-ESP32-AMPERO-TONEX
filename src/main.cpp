...
static short valmaxcc [57] = {0,
  ...
  127,130,127,127,140,
  127,127,127,127,140,127,
  130,4,127,127,127,127,
  127,127,127,127,127,127,127,
  61,150,300
};

static const char* verbname[6] = {  "", 
  "SPRG1", "SPRG2", "SPRG3", " ROOM", "PLATE"
};
  ...
  switch (choixscreen) {
    ...
    case 1 :
      ...
      if (count != 55 && count != 56) {
        ...
        if(count < 18){
          if (params[id][count] < 128 && count != 42) {LCD.print(params[id][count]); val2 = -1;} 
        }
        if(count > 17 && count < 54 && count !=42){
          if (params[id][count] < 128) {
            LCD.setCursor(11,1); LCD.print("     "); LCD.setCursor(11,1);
            texte2 = String((params[id][count]+1)*valminimax[1][count]/valmaxcc[count]);
            LCD.print(texte2); val2 = -1;} 
        }
        if (count == 42) {LCD.setCursor(10,1); LCD.print(verbname[params[id][count]]); val2 = -1;} 
      }
  ...
  while (menus == 1) {
    ...
    if (enc2last != encoder2.getCount()) {
      if (count != 55 && count != 56) {
        ...
        if (params[id][count] < 128 && params[id][count] != 42){
          if (params[id][count] < 128) {tmp = -1; encvalMax = valmaxcc[count]; encvalMini = 0; }
        }
        if (params[id][count] == 42){
          tmp = -1; encvalMax = valminimax[0][count]; valminimax[1][count];
        }
      } 
      else {
